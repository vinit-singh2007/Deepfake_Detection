...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        localStorage.setItem('user', JSON.stringify({ email: formData.email, name: formData.name || formData.email.split('@')[0] }));
      }
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 dark:bg-[#121526] text-slate-900 dark:text-white flex items-center justify-center px-4 font-sans pt-16 transition-colors duration-200">
      {/* Background Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[450px] h-[450px] bg-indigo-600/10 dark:bg-indigo-600/15 rounded-full blur-[140px] pointer-events-none"></div>
      ...
      <div className="relative z-10 w-full max-w-md bg-white dark:bg-[#1a1d36]/90 border border-slate-200 dark:border-purple-500/20 rounded-3xl p-8 shadow-xl dark:shadow-2xl backdrop-blur-xl transition-colors duration-200">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Start verifying video authenticity with DeepShield</p>
        </div>
